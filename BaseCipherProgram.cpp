...
std::vector<int> rotate(int amt, std::vector<int> initial)
{
  int counter = 0;
  char temp;
  while (counter < amt)
...
int main()
{
  ...
  std::vector<int> pv3; //resultant plugboard vector
  ...
  if (!plugboard.is_open()) // error checking for a plugboard file
  ...
  else
    {
      ...
      for(int i = 0;i < pb1.length(); i++)
        {
	  pv1.push_back((int)pb1.at(i));
        }
      for (int i = 0;i < pb2.length(); i++)
        {
	  pv2.push_back((int)pb2.at(i));
        }
      pv3.assign(128, -1);
  ...
  int rotorPosition;
  std::vector <int> rotor1;
  std::vector <int> rotor2;
  std::vector <int> rotor3;
  ...
  std::string useri, ascordec;
  std::cout << "Are you inputting ASCII or decimal representations separated by space (A or D):\n";
  std::cin >> ascordec;
  ...
  std::vector<int> userInVec;

  //gets and converts user input into a vector of characters, for ease of replacement.
  if(ascordec == "A")
    {
